...
int addDLElement(DoublyList* pList, int position, DoublyListNode element)
{
    ...
    newNode->arrivalTime = element.arrivalTime;
    newNode->serviceTime = element.serviceTime;
    newNode->startTime = element.startTime;
    newNode->endTime = element.endTime;
...
void            displayDL(DoublyList *pList)
{
    ...
    for (; curr != pList->tailerNode; curr = curr->pRLink)
        printf("현재 고객 상태\n도착시간\t:\t%d\n서비스 시간\t:\t%d\n서비스 시작시간\t:\t%d\n서비스 종료시간\t:\t%d\n\t⟱\n", curr->arrivalTime, curr->serviceTime, curr->startTime, curr->endTime);
//     printf("\n\n\n");
}
